...
import { buildDirectDispatchAnalysisFromJesusDecision, runOnce } from "../../src/core/orchestrator.js";
...
describe("orchestrator — direct dispatch from Jesus directive", () => {
  it("synthesizes quality-only plans without requiring Prometheus", async () => {
    ...
    try {
      const analysis = await buildDirectDispatchAnalysisFromJesusDecision(config, {
...
describe("orchestrator — fulfilled target short-circuit", () => {
  it("archives a fulfilled target before Jesus spends a premium request", async () => {
    ...
    try {
      ...
      assert.ok(!progress.includes("agent=jesus reason=cycle_directive"));
