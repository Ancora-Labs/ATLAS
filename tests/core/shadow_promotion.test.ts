...
describe("shadow promotion", () => {
  it("promotes a single-target shadow session to active after the last clean batch of wave 1", async () => {
  ...
  it("does not promote before the final batch of wave 1 or on non-done outcomes", () => {
    const config = {
    ...
    assert.equal(shouldPromoteShadowSessionAfterCleanWave({
      ...
      batch: { wave: 1 },
      workerResult: { status: "done" },
      workerBatches: [{ wave: 1 }, { wave: 1 }, { wave: 2 }],
      completedBatchIndex: 0,
    }), false);
