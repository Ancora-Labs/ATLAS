...
## OPERATOR OBJECTIVE
SITUATION: Five PRs merged in rapid succession (PRs #126-#130) all touching gate semantics, policy promotion, SLO retune feedback, adversarial replay, and drift debt deduplication. CI is now green on main. However, internal system health is critical, parser confidence is 0.5 and degrading, and you produced 0 plans in the last cycle. This is the core problem to solve.

PRIMARY MISSION: Diagnose and resolve the parser confidence degradation.
- Parser confidence at 0.5 degrades planning quality. Below 0.4 the planning loop will effectively break.
- Identify what changed in the recent PR surge that is causing parser confidence to degrade: new file structures, changed module exports, renamed symbols, schema changes, missing type annotations, or structural patterns the parser doesn't recognize.
- Check if src/providers/, src/ worker files, or any gate/policy files introduced in #126-#129 follow different structural conventions than existing code.
- The zero plans produced metric is almost certainly a downstream symptom of the parser issue — fix the root cause.

SECONDARY MISSION: Cross-PR coherence validation for PRs #126-#129.
- PR #126 (feat(drift)): Improved debt prioritization, deduplication, and gate semantics.
- PR #127 (feat): T1/T2/T3 adversarial replay corpus, ROI persistence, dispatch command gate.
- PR #128 (feat): Sustained SLO breach signatures → bounded, provenance-tagged retune recommendations.
- PR #129 (feat(gate)): Promote repeated unresolved lessons to hard gate policies earlier with explicit reason codes.
- These four PRs all modify or depend on the gate/policy/dispatch layer. Verify that the lesson-promotion logic (#129) is consistent with the gate semantics changes in #126. Verify that the dispatch command gate (#127) is consistent with the hard gate policy promotion (#129). Verify that the SLO retune recommendations (#128) don't bypass or conflict with the new hard gate policies.
- Look for invariant violations: any path where a gate should block but doesn't, or where a lesson is promoted to policy but the policy enforcement is missing downstream.

TERTIARY MISSION: Integration test coverage gap assessment.
- PR #130 expanded integration coverage and locked artifact/env-contract regressions.
- Verify that the new tests in #130 actually cover the NEW gate semantics introduced in #126-#129, not just pre-existing behavior.
- Identify any coverage gaps for: dispatch command gate (new in #127), hard gate policy promotion with reason codes (new in #129), SLO retune recommendations with provenance tags (new in #128).

FILE AREAS TO SCAN:
- src/ — all gate, policy, dispatch, SLO-related modules
- src/providers/ — any provider files modified by #126-#129
- tests/ — integration tests added in #130
- state/ — current system state files for structural anomalies
- Any file touched by #126-#129 where cross-file contracts may have shifted

EXPECTED DELIVERABLES:
1. Root cause of parser confidence degradation with specific file/line references
2. List of coherence issues found across PRs #126-#129 (if any)
3. List of integration test coverage gaps for new gate/policy/SLO features
4. Concrete remediation plan with file paths and change descriptions

ANTI-GOALS: Do not plan new features. Do not re-architect systems that are working. Do not touch anything unrelated to the gate/policy/SLO/dispatch/parser subsystems. Surgical diagnosis and targeted fixes only.
