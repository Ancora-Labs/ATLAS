...
## OPERATOR OBJECTIVE
## Strategic Directive — Parser Confidence Recovery + Post-Merge Stabilization

### Why This Is Critical
Parser confidence is at 0.5 and degrading. Plans produced last cycle = 0. The system has stalled with 6 workers sitting idle. The root cause hypothesis: today's 5 merged PRs (gate policies, SLO retune signatures, adversarial replay corpus, drift gate semantics, integration coverage) introduced new complexity patterns that the parser is failing to handle confidently. Every agent downstream of the parser — you (Prometheus), Athena, Evolution Worker — is operating on degraded input. Shipping more features on top of a degraded parser creates compounding risk.

### Primary Focus: Parser Confidence Root Cause
Investigate what is causing parser confidence to degrade. Key questions:
- What changed in the codebase patterns after the 5 PRs merged today that the parser struggles with?
- Is the degradation in AST parsing, type inference, import resolution, or structural pattern matching?
- Are there specific files or modules from today's merges (replay corpus, gate policies, SLO signatures) that are causing parse failures or low-confidence results?
- Look at: src/providers/, src/ core orchestration files, and any new files introduced by #126-#130

### Secondary Focus: Plans Produced = 0
Prometheus produced 0 plans last cycle. This is either a direct consequence of parser degradation (can't analyze confidently → won't commit to a plan) or an independent issue in the plan production pipeline itself. Identify which it is. If the pipeline has a bug independent of parser confidence, fix it.

### Tertiary Focus: Post-Merge Integration Coherence
Five significant PRs merged today. Validate they compose correctly:
- #129 (gate): repeated unresolved lessons → hard gate policies with reason codes
- #128 (SLO): sustained breach signatures → provenance-tagged retune recommendations  
...
- #130 (test): integration coverage + lock artifact + env-contract regressions
Check for: circular dependencies between gate policy and replay corpus, SLO retune triggering unintended gate escalations, env-contract regressions in the lock artifact path.

### What NOT To Do
- Do NOT plan new features until parser confidence is ≥ 0.8
- Do NOT investigate the CI failure on commit 112c97a — it's already fixed in 5de6599 (current main is green)
- Do NOT do cosmetic refactors or docs updates
- Do NOT expand test coverage further until the core parsing issue is understood

### Expected Deliverables
1. Root cause identification of parser confidence degradation with specific file paths and code patterns
2. A concrete fix plan for the parser (or the plan production pipeline if that's the real issue)
3. Integration coherence assessment for PRs #126-#130 with any identified composition risks
4. Priority-ordered work items for Evolution Worker to execute
