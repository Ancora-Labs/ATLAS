...
## OPERATOR OBJECTIVE
SYSTEM HEALTH IS CRITICAL. Your primary mission this cycle is to diagnose and plan recovery for the parser confidence degradation — NOT to plan new features.

## What We Know
- Parser confidence: 0.5 (degrading trend over last 10 cycles)
- Plans produced last cycle: 0 (you declared critical but produced nothing actionable)
- CI: green on latest commit (5de6599) but failed on 112c97a ~5 minutes before #130 merged
- 5 PRs merged in ~2 hours: #126 (drift/debt), #127 (adversarial replay + ROI + dispatch gate), #128 (SLO breach signatures), #129 (gate policy promotion), #130 (integration test expansion + artifact/env-contract regressions)

## Mandatory Investigation Areas

### 1. Parser Confidence Root Cause (HIGHEST PRIORITY)
- Examine src/ directory structure for any new files, reorganizations, or naming changes introduced by PRs #126–#130
- Check whether import graphs are resolvable — broken or circular imports directly degrade parser confidence
- Look for missing or incorrect TypeScript types, especially in provider files (src/providers/**/*.js) and new gate/SLO modules
- Check tsconfig.json and tsconfig.typecheck.json — any paths changes or new exclusions?
- Identify files with high cross-module coupling that the parser may be struggling to trace
- Look for any 'any' type explosion, missing return types, or inference failures in recently changed files

### 2. Why Zero Plans Were Produced
- Is the plans array construction broken in your own output pipeline?
- Are the work items failing some validation gate that silently drops them?
- Did the previous cycle's analysis find no discrete, actionable units — and if so, why?

### 3. Integration Test Surface Area (PR #130)
- PR #130 added artifact and env-contract regression tests reactively after a CI failure
- Verify in tests/ directory that the new tests actually cover the surface area that caused the CI failure
- Look for gaps: are there integration points between the new gate policies (#129), SLO signatures (#128), adversarial replay (#127), and drift prioritization (#126) that have no test coverage?

### 4. Technical Debt Accumulation from Velocity Burst
- 5 PRs in 2 hours across interconnected systems (gate, SLO, drift, replay, integration) is high-risk velocity
- Look for: incomplete implementations, TODO comments, hardcoded values, missing error handling, missing validation in the recently merged files
- Check box.config.json and policy.json for any consistency issues introduced by recent changes

## Deliverables Expected
- At least 1 plan targeting parser confidence recovery with specific files to fix
- Clear identification of whether the zero-plans issue is a pipeline bug or a data problem
- Assessment of integration test coverage gaps from the velocity burst
- DO NOT produce plans for new features until parser confidence root cause is identified

## Anti-Goals
- Do not scan the entire codebase generically — focus on files touched by PRs #126–#130
- Do not plan new gate or SLO features — the existing ones need to stabilize first
- Do not optimize for plan count — produce only plans you have high confidence in
