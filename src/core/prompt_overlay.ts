...
const STAGE_OVERLAY_LINES = Object.freeze({
  [PROMPT_STAGE.NONE]: Object.freeze([
    ...
    "Stay within the current self_dev runtime behavior unless a future stage overlay is explicitly activated.",
  ]),
...
function resolveEffectivePromptMode(config: any) {
  ...
  const singleTargetEnabled = config?.selfDev?.futureModeFlags?.singleTargetDelivery === true;

  if (requestedMode === PLATFORM_MODE.SINGLE_TARGET_DELIVERY && !singleTargetEnabled) {
    return {
      requestedMode,
      effectiveMode: PLATFORM_MODE.SELF_DEV,
      singleTargetEnabled,
      fellBack: true,
    };
  }
  ...
  return {
    requestedMode,
    effectiveMode: requestedMode || PLATFORM_MODE.SELF_DEV,
    singleTargetEnabled,
    fellBack: false,
  };
}
...
export function buildPromptAssemblySections(input: {
  ...
  if (mode.effectiveMode === PLATFORM_MODE.SELF_DEV) {
    sections.push(section("mode-overlay-self-dev", [
      "## MODE OVERLAY — SELF_DEV",
      `Requested mode: ${mode.requestedMode}`,
      mode.fellBack
        ? "Requested single_target_delivery mode was blocked because the feature flag is disabled; self_dev overlay remains active."
        : "Self_dev overlay is active and remains the default protected mode.",
      ...contract.selfDevSpecific.map((entry: string) => `- ${entry}`),
    ].join("\n")));
  } else {
